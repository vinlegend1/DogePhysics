...
import {
	getDisplacementFromFreeFallNoAirResistance,
	getVelocityFromFreeFallNoAirResistance,
} from "../utils/freeFall";
		...
		<Layout isSim>
			...
			>
				...
				<div className="container my-36">
					...
					<p className="mt-12">
						<Latex>{String.raw`$x = ${s_0[0].toFixed(
							2
						)} + ${v_0[0].toFixed(2)} \cdot ${time.toFixed(
							1
						)} + \frac{1}{2} \cdot 0 \cdot ${time.toFixed(
							1
						)}^{2} = ${x.toFixed(2)} m$`}</Latex>
					</p>
					<p>
						<Latex>{String.raw`$y = ${s_0[1].toFixed(
							2
						)} + ${v_0[1].toFixed(2)} \cdot ${time.toFixed(
							1
						)} + \frac{1}{2} \cdot ${g} \cdot ${time.toFixed(
							1
						)}^{2} = ${y.toFixed(2)} m$`}</Latex>
					</p>
				</div>
			...
			>
				...
				<Slider
					...
					onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
						...
						setX(
							s_0[0] + v_0[0] * parseFloat(e.currentTarget.value)
						);
						setY(
							getDisplacementFromFreeFallNoAirResistance(
								s_0[1],
...
export const getStaticProps: GetStaticProps = async () => {
	const rawContents = fs
		.readFileSync(path.join("src", "hiw", "free-fall.md"))
		.toString();
